  ...
  factory :review do
    ...
    trait :cainz_nekojarashi do
      transient do
        ...
        sequence(:image) { |n| "cainz_image_url_#{n}"}
      end
      after(:build) do |review, evaluator|
        nekojarashi_category = Category.find(1)
        cainz = create(:cainz, name: evaluator.name, url: evaluator.url, image: evaluator.image)
    ...
    trait :cainz_nezumi do
      transient do
        ...
        sequence(:image) { |n| "cainz_nezumi_image_#{n}"}
      end
      after(:build) do |review, evaluator|
    ...
    trait :rakuten_kerigurumi do
      transient do
        ...
        sequence(:image) { |n| "rakuten_kerigurumi_image_#{n}"}
      end
      after(:build) do |review, evaluator|
        kerigurumi_category = Category.find(3)
        rakuten = create(:rakuten, name: evaluator.name, url: evaluator.url, image: evaluator.image)
    ...
    trait :rakuten_tonnel do
      transient do
        ...
        sequence(:image) { |n| "rakuten_tonnel_image_#{n}"}
      end
      after(:build) do |review, evaluator|
    ...
    trait :rakuten_other do
      transient do
        ...
        sequence(:image) { |n| "rakuten_other_image_#{n}"}
      end
      after(:build) do |review, evaluator|
